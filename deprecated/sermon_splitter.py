...
class VideoProcessor:
    ...
    def burn_subtitles_into_video(self, input_mp4: Path, ass_path: Path, out_path: Path):
...
class Transcriber:
    ...
    def convert_srt_to_styled_ass(self, in_srt_path: Path, out_ass_path: Path):
        ...
        for sub in subs:
            text = self._escape_ass_text(sub.content)
            if not text:
                continue
            start = self._to_ass_timestamp(sub.start)
            end = self._to_ass_timestamp(sub.end)
            ass_lines.append(
                f"Dialogue: 0,{start},{end},SocialCaption,,0,0,0,,{text}"
            )
...
class SermonSplitterApp:
    ...
    def run(self, num_clips, clips_data, output_filename):
        ...
        self.transcriber.convert_srt_to_styled_ass(hinglish_srt, ass_out)

        print("[STEP] Burning subtitles...")
        self.video_processor.burn_subtitles_into_video(final_clip, ass_out, subbed_out)
